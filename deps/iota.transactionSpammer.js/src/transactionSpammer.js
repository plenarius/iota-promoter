    ...
    function reattachTransaction(bundle, inputs){  
        iota.api.isReattachable(inputs,function(error,reattachable){
            ...
            if (reattachable){
              ...
              iota.api.replayBundle(bundle[0].hash,3,14,function(error, new_bundle){
                  if(error){
                      eventEmitter.emitEvent('state', ['Error replaying bundle'])
                      return findPromotableTransaction()
                  }
                  reattachedCount += 1
                  eventEmitter.emitEvent('state', ['Transaction reattached successfully, now attempting promote.'])
                  eventEmitter.emitEvent('transactionCountChanged', [promotedCount,reattachedCount])

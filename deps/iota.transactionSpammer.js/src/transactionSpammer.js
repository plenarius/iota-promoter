...
window.iotaTransactionSpammer = (function(){
    ...
    const BUNDLE_SIZE_LIMIT = 25 // larger bundles can eat up resources
    curl.init()
    ...
    function findPromotableTransaction(){
        var tip
        var value = 0
        ...
        while(value < 1 || transactionAge < promoteTimeThreshold){
            ...
            if (value < 1) {
                zeroValueCount++
                zeroValueTips.push(tip.hash)
                eventEmitter.emitEvent('rejectedCountChanged', [zeroValueCount,tooNewCount,unpromotableCount,unreattachableCount,tooBigCount])
            }
        ...
        iota.api.getBundle(tip.hash,(error,bundle) => {
            if(error){
                return findPromotableTransaction(tips)
            }
            ...
            iota.api.isPromotable(bundle[0].hash).then(state => {
              if (state) {
                ...
                promoteTransaction(bundle)
              } else { // Categorize transactions that need to be reattached
                unpromotableCount++
                unpromotableTips.push(tip.hash)
                eventEmitter.emitEvent('rejectedCountChanged', [zeroValueCount,tooNewCount,unpromotableCount,unreattachableCount,tooBigCount])
                if (bundle.length >= BUNDLE_SIZE_LIMIT) {
                  tooBigCount++
                  tooBigTips.push(tip.hash)
                  eventEmitter.emitEvent('rejectedCountChanged', [zeroValueCount,tooNewCount,unpromotableCount,unreattachableCount,tooBigCount])
                  eventEmitter.emitEvent('state', ['Not promotable, bundle is too big for reattach.'])
                  return findPromotableTransaction(tips)
                }
                eventEmitter.emitEvent('state', ['Not promotable, will try reattaching instead.']) 
                let inputs = bundle
                  .filter(input => input.value < 0)
                  .map(input => input.address);
                reattachTransaction(bundle, inputs)
              }
            })
        })
    }

    function promoteTransaction(bundle){
        var count = 0;
        ...
        iota.api.promoteTransaction(bundle[0].hash,3,14,[transfer],params,(error, success) => {
            if(error){
                unpromotableCount++
                unreattachableCount++
                unpromotableTips.push(bundle[0].hash)
                eventEmitter.emitEvent('rejectedCountChanged', [zeroValueCount,tooNewCount,unpromotableCount,unreattachableCount,tooBigCount])
            ...
            eventEmitter.emitEvent('transactionCountChanged', [promotedCount,reattachedCount])
            eventEmitter.emitEvent('transactionCompleted', [bundle[0].hash])
            promotedTips.push(bundle[0].hash)
            checkIsConfirmed(bundle,Date.now())
    ...
    function reattachTransaction(bundle, inputs){  
        iota.api.isReattachable(inputs,(error,reattachable) => {
            if (error){
                unreattachableCount++
                eventEmitter.emitEvent('state', ['Error occurred while checking if transaction reattachable'])
                  eventEmitter.emitEvent('rejectedCountChanged', [zeroValueCount,tooNewCount,unpromotableCount,unreattachableCount,tooBigCount])
                return findPromotableTransaction()
            }
            ...
            if (reattachable){
              ...
              iota.api.replayBundle(bundle[0].hash,3,14,(error, new_bundle) => {
                  if(error){
                  ...
                  eventEmitter.emitEvent('transactionCountChanged', [promotedCount,reattachedCount])
                  promoteTransaction(new_bundle)
              })
            }
            else{
                unreattachableCount++
                eventEmitter.emitEvent('state', ['Not reattachable, moving on.'])
                eventEmitter.emitEvent('rejectedCountChanged', [zeroValueCount,tooNewCount,unpromotableCount,unreattachableCount])
                findPromotableTransaction()
            }
        })
    }

    function checkIsConfirmed(bundle,startTime){
        let inputs = bundle
          .filter(input => input.value < 0)
          .map(input => input.address);
        iota.api.isReattachable(inputs,(error,reattachable) => {
            if (error){
                ...
                setTimeout(checkIsConfirmed,30000,bundle,startTime)   
                return;
            }
            if (typeof(reattachable) === 'object' && reattachable.some(x => !x)) {
              reattachable = false;
            ...
            if (!reattachable){ 
                confirmationCount += 1
                transactionDuration = (Date.now() - startTime)/60
                confirmationTimes.push(transactionDuration)
                var sum = confirmationTimes.reduce((a, b) => a + b, 0);
    ...
    function populateTips(){
        ...
        iota.api.getTips((error,success) => {
            if(error){
                ...
                return checkIfNodeIsSynced()
            }
            tips = success
            tipTransactions = []
            tipIndex = 0
            return getTipTransactionObjects()
        })
    }
    ...
    function getTipTransactionObjects(){
        var tipSlice
        var done = false
        var rejectedTips = zeroValueTips.concat(unpromotableTips, tooBigTips)
        // Remove rejected for zero value
        tips = tips.filter( function( el ) {
          return !rejectedTips.includes( el );
        } );
        // Remove already promoted
        tips = tips.filter( function( el ) {
          return !promotedTips.includes( el );
        } );
        ...
        iota.api.getTransactionsObjects(tipSlice,(error,success) => {
            if(error){
                eventEmitter.emitEvent('state', ['Error getting tips.'])
                return
            }
            ...
            if(!done){
                ...
                return setTimeout(getTipTransactionObjects,100)
            }
            else{
                tipTransactions = shuffle(tipTransactions)
                return restartSpamming()
            }
        })
    }
    ...
    return {
        ...
        getTransactionCount: () => promotedCount,
